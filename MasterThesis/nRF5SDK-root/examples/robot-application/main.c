...
/*	Template for writing to microsd card. If logging faster than 400ms to sd card it can 
    slow the robot down considerably.  
...
*/
    ...
    /** mag_init(MAG_OS_128);//oversampling rate used to set datarate 
     * 16->80hz 32->40hz 64->20hz 128->10hz
    ...
    while(true){
		...
		/**
         * Test-function, sends targetX and targetY to controller some time after initialization, 
         * used to test waypoints without server running.
        */
		if(testWaypoint){
			...
			if ((time > 40) && (sent == false)){
				...
				xQueueSend(poseControllerQ, &target, 100);  // Sends target to poseControllerQ, 
                                                // which is received and handled by ControllerTask
				sent = true;
				time = 0;
			}
		}
...
int main(void) {
    bsp_board_init(BSP_INIT_LEDS); // SDK
    bool erase_bonds;
    clock_init();
    ir_init();
...
// RecorderDataPtr
   ...
   //error_msg = xTraceGetLastError();
	...
	xPoseMutex = xSemaphoreCreateMutex();       // Global variables for robot pose. 
                                                //Only updated from estimator, accessed from many
    ...
    if (pdPASS != xTaskCreate(user_task, "USER", 128, NULL, 4, &handle_user_task)) 
        //needs elevated priority because init functions
        APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
    ...
    */
    ...
    if (pdPASS != xTaskCreate(vMainCommunicationTask, "COM", 256, NULL, 3, &communication_task))
        // Moved to this loop in order to use it for thread communications aswell
            APP_ERROR_HANDLER(NRF_ERROR_NO_MEM); 
    ...
    if (USEBLUETOOTH)
    {
        if(pdPASS != xTaskCreate(vARQTask, "ARQ", 256, NULL, 3, &arq_task) ) { 
            NRF_LOG_INFO("vARQTask Creation Failed");
            APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
        }
        /** 
        * Suspend arq task to avoid start before init and avoids init 
        * before freertos start at same time
        */
